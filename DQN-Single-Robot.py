...
class Qnetwork():
	def __init__(self, lr, s_size,a_size,h_size, batch_size, myScope):
		...
		self.hidden2 = slim.fully_connected(self.hidden,int(h_size/2),biases_initializer=None,weights_initializer = initializer, activation_fn=tf.nn.relu)
...
with tf.Session() as sess:
	if training:
		...
		for i in range(num_episodes):
			...
			s = target_observation_to_input_array(step.ai_data_input,target)
			d = False
			xList = []
			rAll = 0
			j = 0
			...
			while True: #If the agent takes longer than max time, end the trial.
				...
				a = sess.run(mainQN.predict,feed_dict={mainQN.inputs:[s],mainQN.keep_per2:1, mainQN.keep_per:1})
				a = a[0]
				r = 0 #_sim.action_rewards(action_pool[a])/1000;

				_sim.target_send_command_gui(action_pool[a], target)
				step = _sim.recieve_state_gui()
				while step.ai_data_input.elapsed_time - last_time < 5 or not step.cmd_done:
					r += step.reward
					step = _sim.recieve_state_gui()
				...
				if step.ai_data_input.target_removed[target]:
					...
					print(step.ai_data_input.target_y[target])	
					target = chooseRobot(step.ai_data_input)
				s1 = target_observation_to_input_array(step.ai_data_input,target);
